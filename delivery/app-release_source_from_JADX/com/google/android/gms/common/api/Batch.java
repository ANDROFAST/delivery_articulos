package com.google.android.gms.common.api;

import com.google.android.gms.common.api.PendingResult.zza;
import com.google.android.gms.internal.zzly;
import java.util.ArrayList;
import java.util.List;

public final class Batch extends zzly<BatchResult> {
    private int zzaes;
    private boolean zzaet;
    private boolean zzaeu;
    private final PendingResult<?>[] zzaev;
    private final Object zzpK;

    public static final class Builder {
        private GoogleApiClient zzYC;
        private List<PendingResult<?>> zzaex = new ArrayList();

        public Builder(GoogleApiClient googleApiClient) {
            this.zzYC = googleApiClient;
        }

        public <R extends Result> BatchResultToken<R> add(PendingResult<R> pendingResult) {
            BatchResultToken<R> batchResultToken = new BatchResultToken(this.zzaex.size());
            this.zzaex.add(pendingResult);
            return batchResultToken;
        }

        public Batch build() {
            return new Batch(this.zzaex, this.zzYC);
        }
    }

    class C06431 implements zza {
        final /* synthetic */ Batch zzaew;

        C06431(Batch batch) {
            this.zzaew = batch;
        }

        /* JADX WARNING: inconsistent code. */
        /* Code decompiled incorrectly, please refer to instructions dump. */
        public void zzu(com.google.android.gms.common.api.Status r6) {
            /*
            r5 = this;
            r0 = r5.zzaew;
            r1 = r0.zzpK;
            monitor-enter(r1);
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r0 = r0.isCanceled();	 Catch:{ all -> 0x0039 }
            if (r0 == 0) goto L_0x0011;
        L_0x000f:
            monitor-exit(r1);	 Catch:{ all -> 0x0039 }
        L_0x0010:
            return;
        L_0x0011:
            r0 = r6.isCanceled();	 Catch:{ all -> 0x0039 }
            if (r0 == 0) goto L_0x003c;
        L_0x0017:
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r2 = 1;
            r0.zzaeu = r2;	 Catch:{ all -> 0x0039 }
        L_0x001d:
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r0.zzaes = r0.zzaes - 1;	 Catch:{ all -> 0x0039 }
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r0 = r0.zzaes;	 Catch:{ all -> 0x0039 }
            if (r0 != 0) goto L_0x0037;
        L_0x002a:
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r0 = r0.zzaeu;	 Catch:{ all -> 0x0039 }
            if (r0 == 0) goto L_0x0049;
        L_0x0032:
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            super.cancel();	 Catch:{ all -> 0x0039 }
        L_0x0037:
            monitor-exit(r1);	 Catch:{ all -> 0x0039 }
            goto L_0x0010;
        L_0x0039:
            r0 = move-exception;
            monitor-exit(r1);	 Catch:{ all -> 0x0039 }
            throw r0;
        L_0x003c:
            r0 = r6.isSuccess();	 Catch:{ all -> 0x0039 }
            if (r0 != 0) goto L_0x001d;
        L_0x0042:
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r2 = 1;
            r0.zzaet = r2;	 Catch:{ all -> 0x0039 }
            goto L_0x001d;
        L_0x0049:
            r0 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r0 = r0.zzaet;	 Catch:{ all -> 0x0039 }
            if (r0 == 0) goto L_0x0069;
        L_0x0051:
            r0 = new com.google.android.gms.common.api.Status;	 Catch:{ all -> 0x0039 }
            r2 = 13;
            r0.<init>(r2);	 Catch:{ all -> 0x0039 }
        L_0x0058:
            r2 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r3 = new com.google.android.gms.common.api.BatchResult;	 Catch:{ all -> 0x0039 }
            r4 = r5.zzaew;	 Catch:{ all -> 0x0039 }
            r4 = r4.zzaev;	 Catch:{ all -> 0x0039 }
            r3.<init>(r0, r4);	 Catch:{ all -> 0x0039 }
            r2.zzb(r3);	 Catch:{ all -> 0x0039 }
            goto L_0x0037;
        L_0x0069:
            r0 = com.google.android.gms.common.api.Status.zzaeX;	 Catch:{ all -> 0x0039 }
            goto L_0x0058;
            */
            throw new UnsupportedOperationException("Method not decompiled: com.google.android.gms.common.api.Batch.1.zzu(com.google.android.gms.common.api.Status):void");
        }
    }

    private Batch(List<PendingResult<?>> pendingResultList, GoogleApiClient apiClient) {
        super(apiClient);
        this.zzpK = new Object();
        this.zzaes = pendingResultList.size();
        this.zzaev = new PendingResult[this.zzaes];
        for (int i = 0; i < pendingResultList.size(); i++) {
            PendingResult pendingResult = (PendingResult) pendingResultList.get(i);
            this.zzaev[i] = pendingResult;
            pendingResult.zza(new C06431(this));
        }
    }

    public void cancel() {
        super.cancel();
        for (PendingResult cancel : this.zzaev) {
            cancel.cancel();
        }
    }

    public BatchResult createFailedResult(Status status) {
        return new BatchResult(status, this.zzaev);
    }

    public /* synthetic */ Result zzc(Status status) {
        return createFailedResult(status);
    }
}

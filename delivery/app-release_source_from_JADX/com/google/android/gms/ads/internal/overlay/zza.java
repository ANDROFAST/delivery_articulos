...
public class zza {
    public boolean zza(Context context, AdLauncherIntentInfoParcel adLauncherIntentInfoParcel, zzn com_google_android_gms_ads_internal_overlay_zzn) {
        ...
        if (!TextUtils.isEmpty(adLauncherIntentInfoParcel.zzCK)) {
            String[] split = adLauncherIntentInfoParcel.zzCK.split("/", 2);
            if (split.length < 2) {
                zzb.zzaH("Could not parse component name from open GMSG: " + adLauncherIntentInfoParcel.zzCK);
                return false;
            }
            intent.setClassName(split[0], split[1]);
        }
        Object obj = adLauncherIntentInfoParcel.zzCL;
        ...
        try {
            zzb.m7v("Launching an intent: " + intent.toURI());
            zzp.zzbx().zzb(context, intent);
            if (com_google_android_gms_ads_internal_overlay_zzn != null) {
                com_google_android_gms_ads_internal_overlay_zzn.zzaQ();
            }
            return true;

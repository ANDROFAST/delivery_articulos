...
public class zzbq {
    ...
    static String zza(String[] strArr, int i, int i2) {
        if (strArr.length < i + i2) {
            zzb.m6e("Unable to construct shingle");
            return "";
        }
    ...
    static void zza(int i, long j, String str, PriorityQueue<zza> priorityQueue) {
        zza com_google_android_gms_internal_zzbq_zza = new zza(j, str);
        if ((priorityQueue.size() != i || ((zza) priorityQueue.peek()).value <= com_google_android_gms_internal_zzbq_zza.value) && !priorityQueue.contains(com_google_android_gms_internal_zzbq_zza)) {
            priorityQueue.add(com_google_android_gms_internal_zzbq_zza);
    ...
    public static void zza(String[] strArr, int i, int i2, PriorityQueue<zza> priorityQueue) {
        ...
        for (int i3 = 1; i3 < (strArr.length - i2) + 1; i3++) {
            zzb = zza(zzbo.zzC(strArr[i3 - 1]), zzbo.zzC(strArr[(i3 + i2) - 1]), zzb, zza, 16785407);
            zza(i, zzb, zza(strArr, i3, i2), (PriorityQueue) priorityQueue);
        }
    }

    private static long zzb(String[] strArr, int i, int i2) {
        long zzC = (((long) zzbo.zzC(strArr[i])) + 2147483647L) % 1073807359;
        for (int i3 = i + 1; i3 < i + i2; i3++) {
            zzC = (((zzC * 16785407) % 1073807359) + ((((long) zzbo.zzC(strArr[i3])) + 2147483647L) % 1073807359)) % 1073807359;
        }
        return zzC;
    }
}

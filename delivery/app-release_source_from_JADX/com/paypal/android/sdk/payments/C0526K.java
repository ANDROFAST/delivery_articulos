package com.paypal.android.sdk.payments;

import android.content.Context;
import android.view.View;
import android.webkit.WebView;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.ScrollView;
import android.widget.TextView;
import com.paypal.android.sdk.C0510f;
import com.paypal.android.sdk.bo;
import com.paypal.android.sdk.bp;
import com.paypal.android.sdk.cB;
import com.paypal.android.sdk.cz;
import java.util.Arrays;
import java.util.HashSet;
import java.util.Locale;

final class C0526K {
    View f728a;
    TextView f729b;
    Button f730c;
    private ScrollView f731d;
    private LinearLayout f732e;
    private ImageView f733f;

    C0526K(Context context, C0525J c0525j) {
        String format;
        this.f731d = new ScrollView(context);
        this.f731d.setBackgroundColor(bo.f342d);
        this.f732e = new LinearLayout(context);
        this.f732e.setOrientation(1);
        this.f731d.addView(this.f732e);
        bp.m309a(this.f732e, -1, -2);
        View linearLayout = new LinearLayout(context);
        linearLayout.setOrientation(1);
        bp.m314a(linearLayout, "16dip", "20dip", "16dip", "20dip");
        this.f732e.addView(linearLayout);
        bp.m309a(linearLayout, -1, -2);
        this.f733f = new ImageView(context);
        this.f733f.setImageBitmap(bp.m325c("", context));
        this.f733f.setContentDescription("PayPal");
        linearLayout.addView(this.f733f);
        bp.m310a(this.f733f, -1, "40dip");
        bp.m322b(this.f733f, null, null, null, "10dip");
        this.f729b = new TextView(context);
        View webView = new WebView(context);
        webView.setBackgroundColor(bo.f342d);
        StringBuilder append = new StringBuilder("<html><meta name='viewport' content='width=device-width; initial-scale=1.0;'/><head><style type=\"text/css\">").append("body { color: #515151; line-height: 26px; font-size: 18px; font-family: sans-serif-light, DroidSans, sans-serif; font-weight: 300; " + (cz.f622a ? "direction: rtl;" : "") + "} h1, strong { font-family: DroidSans, sans-serif; font-weight: 500; }" + "h1 { font-size: 18px }h1, p { margin: 0 0 13px 0 }" + "ul { margin: 0; padding-left: 1.1em; list-style-position: outside; }").append("</style></head><body>");
        switch (C0527L.f734a[c0525j.ordinal()]) {
            case 1:
                String a = cz.m428a(cB.FUTURE_PAYMENT_LEGAL_DETAILS);
                Object toLowerCase = Locale.getDefault().getCountry().toLowerCase(Locale.US);
                if (C0510f.m464c((CharSequence) toLowerCase)) {
                    toLowerCase = "us";
                }
                format = new HashSet(Arrays.asList(new String[]{"nl", "de", "it", "fr", "gb", "us"})).contains(toLowerCase) ? String.format("https://www.paypal.com/%s/webapps/mpp/ua/useragreement-full", new Object[]{toLowerCase}) : "";
                format = String.format(a, new Object[]{format});
                break;
            case 2:
                format = cz.m428a(cB.FINANCIAL_INSTRUMENTS_LEGAL_DETAILS);
                break;
            case 3:
                format = String.format(cz.m428a(cB.REQUEST_OR_SEND_MONEY_LEGAL_DETAILS), new Object[]{cz.m428a(cB.REQUEST_MONEY)});
                break;
            case 4:
                format = String.format(cz.m428a(cB.REQUEST_OR_SEND_MONEY_LEGAL_DETAILS), new Object[]{cz.m428a(cB.SEND_MONEY)});
                break;
            default:
                format = null;
                break;
        }
        webView.loadData(append.append(format).append("</body><html>").toString(), "text/html; charset=UTF-8", null);
        webView.setClickable(true);
        linearLayout.addView(webView);
        bp.m309a(webView, -1, -2);
        bp.m322b(webView, null, null, null, "10dip");
        this.f730c = new Button(context);
        linearLayout.addView(this.f730c);
        bp.m315a(this.f730c, true, context);
        bp.m322b(this.f730c, null, "20dip", null, "4dip");
        this.f728a = this.f731d;
    }
}

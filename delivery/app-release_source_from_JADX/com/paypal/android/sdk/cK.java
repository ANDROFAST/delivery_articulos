package com.paypal.android.sdk;

import android.content.Context;
import android.util.TypedValue;
import android.view.View;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.ImageView.ScaleType;
import android.widget.LinearLayout;
import android.widget.TextView;
import com.google.android.gms.cast.TextTrackStyle;

public final class cK {
    public ViewGroup f515a;
    public Button f516b;
    public cP f517c;
    public TextView f518d;
    public TextView f519e;
    public TextView f520f;
    public ImageView f521g;
    public LinearLayout f522h;
    public cD f523i;
    public LinearLayout f524j;
    public Button f525k;
    public TextView f526l;
    public ImageView f527m;
    private LinearLayout f528n = bp.m304a(this.f515a);

    public cK(Context context) {
        this.f515a = bp.m301a(context);
        LinearLayout b = bp.m320b(this.f528n);
        this.f526l = new TextView(context);
        this.f526l.setText("Your Order");
        bp.m314a(this.f526l, "0dip", "0dip", "0dip", "14dip");
        this.f526l.setTextSize(24.0f);
        this.f526l.setTextColor(bo.f339a);
        b.addView(this.f526l, new LayoutParams(-2, -2));
        this.f517c = new cP(context, "$0.00");
        b.addView(this.f517c.f546b);
        bp.m313a(this.f517c.f546b, null, "16dip");
        this.f524j = bp.m303a(context, true, 42044, b);
        View imageView = new ImageView(context);
        imageView.setImageBitmap(bp.m325c("", context));
        imageView.setScaleType(ScaleType.CENTER_INSIDE);
        imageView.setAdjustViewBounds(true);
        imageView.setContentDescription("PayPal");
        if (cz.f622a) {
            this.f524j.addView(imageView);
            bp.m322b(imageView, "4dip", "2.5dip", null, null);
            bp.m310a(imageView, -2, "28dip");
        }
        this.f520f = new TextView(context);
        bp.m317a(this.f520f);
        this.f520f.setText("Pay with");
        this.f524j.addView(this.f520f);
        if (!cz.f622a) {
            this.f524j.addView(imageView);
            bp.m322b(imageView, "4dip", "2.5dip", null, null);
            bp.m310a(imageView, -2, "28dip");
        }
        imageView = new LinearLayout(context);
        imageView.setOrientation(1);
        b.addView(imageView, new LinearLayout.LayoutParams(-1, -2));
        bp.m322b(imageView, null, null, null, "16dip");
        this.f518d = new TextView(context);
        this.f518d.setTextSize(14.0f);
        this.f518d.setTextColor(bo.f350l);
        this.f518d.setGravity(17);
        imageView.addView(this.f518d, new LinearLayout.LayoutParams(-1, -2));
        bp.m322b(this.f518d, null, "8dip", null, "4dip");
        this.f516b = new Button(context);
        imageView.addView(this.f516b);
        bp.m316a(this.f516b);
        bp.m310a(this.f516b, -2, "24dip");
        bp.m308a(this.f516b, 17, (float) TextTrackStyle.DEFAULT_FONT_SCALE);
        this.f522h = bp.m303a(context, false, 42043, b);
        this.f527m = new ImageView(context);
        this.f527m.setAdjustViewBounds(true);
        this.f527m.setContentDescription("PayPal");
        this.f527m.setScaleType(ScaleType.CENTER_INSIDE);
        this.f527m.setImageBitmap(bp.m325c("", context));
        this.f522h.addView(this.f527m, new LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        bp.m322b(this.f527m, null, null, "6dip", "1dip");
        bp.m310a(this.f527m, -2, "22dip");
        this.f527m.setVisibility(8);
        this.f521g = new ImageView(context);
        this.f521g.setAdjustViewBounds(true);
        this.f521g.setContentDescription("PayPal");
        this.f521g.setScaleType(ScaleType.CENTER_INSIDE);
        this.f521g.setVisibility(8);
        this.f522h.addView(this.f521g, new LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        bp.m322b(this.f521g, null, null, "4dip", null);
        bp.m321b(this.f521g, "36dip", "25dip");
        this.f519e = new TextView(context);
        bp.m317a(this.f519e);
        this.f519e.setText("Pay with Card");
        this.f522h.addView(this.f519e, new LayoutParams(-2, -2));
        this.f525k = new Button(context);
        this.f525k.setId(42042);
        bp.m316a(this.f525k);
        b.addView(this.f525k);
        bp.m322b(this.f525k, null, null, null, "16dip");
        bp.m309a(this.f525k, -2, -2);
        bp.m308a(this.f525k, 17, (float) TextTrackStyle.DEFAULT_FONT_SCALE);
        this.f523i = new cD(context);
        b.addView(this.f523i.f471a);
        bp.m309a(this.f523i.f471a, -2, -2);
        bp.m308a(this.f523i.f471a, 17, (float) TextTrackStyle.DEFAULT_FONT_SCALE);
    }
}
